...
const listingsCollection = collection(firestore, COLLECTION_NAME);
...
export const getListings = async (): Promise<Listing[]> => {
  try {
    ...
    return querySnapshot.docs.map((doc) => {
      // Using type assertion here is necessary because Firestore data doesn't exactly match our type
      return {
        id: doc.id,
        ...doc.data()
  ...
  try {
    ...
    if (sortBy === 'listPrice') {
      ...
      if (criteria.minBedrooms !== undefined && criteria.minBedrooms > 0) {
        const minBeds = criteria.minBedrooms;
        clientSideFilters.push((listing: Listing) =>
          listing.bedrooms >= minBeds
        );
      }
    } else {
      ...
      if (criteria.minPrice !== undefined && criteria.minPrice > 0 &&
        ...
        clientSideFilters.push((listing: Listing) =>
          listing.listPrice >= minPrice &&
          listing.listPrice <= maxPrice
        );
